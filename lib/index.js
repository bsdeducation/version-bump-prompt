...
function getManifests (options) {
  ...
  return candidates.filter((manifest) => {
    ...
    try {
      ...
      return pkg.hasOwnProperty(options.versionAttributeName);
    }
    catch (err) {
      return false;
    }
...
function versionInfo (manifest, options) {
  ...
  let current = new SemVer(pkg[options.versionAttributeName] || "0.0.0");
...
function bump (manifest, type, options) {
  ...
  oldVersion = pkg[options.versionAttributeName] || "0.0.0";
  ...
  if (newVersion !== oldVersion) {
    ...
    pkg[options.versionAttributeName] = newVersion;
